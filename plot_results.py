...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--overlay-scores", 
        action="store_true",
        help="Overlay baseline and post-removal accuracies as connected "
             "markers above bars."
    )
    p.add_argument(
        "--no-control", 
...
def load_metrics_json(path: Path) -> Dict[str, Any]:
    try:
        ...
        if len(path.parents) >= 3:
            data["_run_dir"] = str(path.parent.relative_to(path.parents[2]))
        else:
...
def plot_all(df: pd.DataFrame, out_dir: Path, show: bool, sort_by: str, *,
    ...
    final_agg_spec = {
        k: v for k, v in agg_spec.items() if v[0] in df.columns
    }
    
    if "method" in df.columns:
        df = df[df["method"].fillna("eap") == "eap"].copy()
    
    grouped = (df
               .groupby(["model_display", "task_display"], as_index=False)
               .agg(**final_agg_spec))
    
    # Calculate accuracy means and error bars
    for split in ["train", "val"]:
        for cat in ["baseline", "ablation", "control"]:
            ...
            if correct_col in grouped.columns and total_col in grouped.columns:
                grouped[acc_col] = (grouped[correct_col] / 
    ...
    # Check if aggregation averaged over varying parameters
    if ({"top_k", "model_name"}.intersection(df.columns) and 
        (df[["model_name", "task"]].drop_duplicates().shape[0] != 
         grouped.shape[0])):
    ...
    def _create_plot(sub: pd.DataFrame,
        ...
        if (sort_by == "drop" and 
            (baseline_col in sub.columns) and 
            (ablation_col in sub.columns)):
            sub = sub.assign(
        ...
        has_control = (control_col is not None and 
                      (control_col in sub.columns))
        ctrl_vals = (sub[control_col].values * scale 
        ...
        base_err = (sub[base_err_col].values * scale 
                   if base_err_col in sub.columns and error_bars_enabled 
                   else None)
        abl_err = (sub[abl_err_col].values * scale 
                  if abl_err_col in sub.columns and error_bars_enabled 
                  else None)
        ...
        error_kwargs = (dict(elinewidth=1.2, capsize=4, capthick=1.2, 
                           ecolor="black") 
                       if error_bars_enabled else {})
        
        # Create bars
        bar_containers: List[Any] = []
        for off, lab, vals, err, col in zip(offsets, labels, value_arrays, 
                                          error_arrays, colors):
            bc = ax.bar(
                x + off,
                vals,
        ...
        if percent:
            ...
            max_val = (float(np.max([np.max(v) for v in value_arrays 
                                   if v is not None])) 
                      if value_arrays else 1.0)
        ...
        # Add overlay scores if requested
        if overlay_scores:
            ax.plot(x, base_vals, marker="o", color=base_palette[0], 
                   linewidth=1, label="_nolegend_")
            ax.plot(x, abl_vals, marker="s", color=base_palette[1], 
                   linewidth=1, label="_nolegend_")
            if has_control and ctrl_vals is not None:
                ax.plot(x, ctrl_vals, marker="^", color=base_palette[2], 
                       linewidth=1, label="_nolegend_")
    ...
    for model_disp in sorted(grouped.model_display.unique()):
        ...
        # Train/Train plot
        if (("baseline_train_accuracy_mean" in sub.columns) and 
            ("ablation_train_accuracy_mean" in sub.columns)):
            print(f"[PLOT] {model_disp} - {pretty_method} Train/Train")
            control_col = ("control_train_accuracy_mean" 
                          if include_control and 
                             ("control_train_accuracy_mean" in sub.columns)
                          else None)
            _create_plot(
                sub,
        ...
        # Val/Val plot
        if (("baseline_val_accuracy_mean" in sub.columns) and 
            sub["baseline_val_accuracy_mean"].notna().any()):
            print(f"[PLOT] {model_disp} - {pretty_method} Val/Val")
            control_col = ("control_val_accuracy_mean" 
                          if include_control and 
                             ("control_val_accuracy_mean" in sub.columns)
                          else None)
            _create_plot(
                sub,
        ...
        # Train/Val plot
        train_val_condition = (
            ("baseline_train_accuracy_mean" in sub.columns) and 
            sub["baseline_train_accuracy_mean"].notna().any() and
            ("ablation_val_accuracy_mean" in sub.columns) and 
            sub["ablation_val_accuracy_mean"].notna().any()
        )
        if train_val_condition:
        ...
        # Circuit reuse plot
        reuse_condition = (
            ("shared_circuit_size_mean" in sub.columns and 
             sub["shared_circuit_size_mean"].notna().any()) and
            ("top_k_mean" in sub.columns and 
             sub["top_k_mean"].notna().any())
        )
        if reuse_condition:
            reuse_df = sub[["model_display", "task_display", 
                           "shared_circuit_size_mean", "top_k_mean"]].copy()
            reuse_df = reuse_df[reuse_df["top_k_mean"] > 0]
            
            if not reuse_df.empty:
                print(f"[PLOT] {model_disp} - {pretty_method} Circuit Reuse")
                
                reuse_df["reuse_fraction"] = (
                    reuse_df["shared_circuit_size_mean"] / 
                    reuse_df["top_k_mean"].clip(lower=1e-9)
                )
                reuse_df["reuse_plot_value"] = (
                    reuse_df["reuse_fraction"] * (100.0 if percent else 1.0)
                )
                
                # Sort reuse data
                if sort_by in {"drop", "baseline", "ablation"}:
                    reuse_df = reuse_df.sort_values(
                        "reuse_plot_value", ascending=False
                    )
                else:
                ...
                line_color = sns.color_palette("colorblind")[
                    3 % len(sns.color_palette("colorblind"))
                ]
                ...
                # Style reuse plot
                ylabel = ("Shared Circuit (% of top_k)" if percent 
                         else "Shared Circuit Fraction")
                ...
                # Save reuse plot
                reuse_fname = f"{safe_model}_eap_reuse.png"
                plt.savefig(out_dir / reuse_fname, dpi=200, 
                           bbox_inches="tight")
