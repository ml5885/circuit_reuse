...
def EAP_clean_backward_hook(
    ...
    hook_slice = graph.get_hook_slice(hook.name)
    earlier_upstream_nodes_slice = graph.get_slice_previous_upstream_nodes(
        hook
    )
    ...
    mat = torch.matmul(
        upstream_activations_difference[
            :, :, earlier_upstream_nodes_slice
        ],
        grad_expanded.transpose(-1, -2),
    )
    result = mat.sum(dim=0).sum(dim=0)

    # result has shape [n_upstream_prev, n_downstream_at_hook]
    # but with GQA, q heads != kv heads. Ensure the second dim matches
    # hook_slice.
    hook_len = hook_slice.stop - hook_slice.start
    if result.shape[1] != hook_len:
        # If queries are grouped into kv heads, average groups
        if result.shape[1] % hook_len == 0:
            group = result.shape[1] // hook_len
            result = result.view(
                result.shape[0],
                hook_len,
                group,
            ).mean(-1)
        # If somehow we have fewer columns, tile to fit (rare)
        elif hook_len % result.shape[1] == 0:
            repeat = hook_len // result.shape[1]
            result = result.repeat(1, repeat)
        else:
            raise RuntimeError(
                f"GQA head-count mismatch: "
                f"result has {result.shape[1]} cols, "
                f"hook_slice expects {hook_len} for "
                f"hook {hook.name}"
            )

    graph.eap_scores[
        earlier_upstream_nodes_slice, hook_slice
    ] += result
...
def EAP_downstream_patching_hook(
    ...
    patch_difference = einops.einsum(
        ...
        (
            "n_upstream n_downstream_at_hook, "
            "batch_size seq_len n_upstream d_model -> "
            "batch_size seq_len n_downstream_at_hook d_model"
        ),
    )
...
def EAP(
    ...
    if clean_tokens.shape[0] != corrupted_tokens.shape[0]:
        raise ValueError(
            f"Mismatched batch size between clean "
            f"({clean_tokens.shape}) and corrupted "
            f"({corrupted_tokens.shape}) tokens"
        )

    common_len = min(clean_tokens.shape[1], corrupted_tokens.shape[1])
    if common_len <= 0:
        raise ValueError(
            f"Non-positive common sequence length: "
            f"clean={clean_tokens.shape}, "
            f"corrupted={corrupted_tokens.shape}"
        )
    ...
    for idx in tqdm(range(0, num_prompts, batch_size)):
        model.add_hook(
            upstream_hook_filter,
            corruped_upstream_hook_fn,
            "fwd",
        )
        ...
        model.add_hook(
            upstream_hook_filter,
            clean_upstream_hook_fn,
            "fwd",
        )
        model.add_hook(
            downstream_hook_filter,
            clean_downstream_hook_fn,
            "bwd",
        )
